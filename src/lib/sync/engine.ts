...
async function publishWith(
  ...
  check(gen);
  return results.some((r) => r.status === 'fulfilled') ? { id: event.id, createdAt } : null;
}
...
export async function changeSyncKey(newPhrase: string): Promise<void> {
  ...
  const done = await queue(async () => {
    ...
    check(gen);
    if (failed > 0) {
      report('offline', "Couldn't upload your log under the new key, so nothing changed. Try again when you're online.");
      throw new Error("Couldn't upload your log under the new key, so nothing changed. Try again when you're online.");
    }
